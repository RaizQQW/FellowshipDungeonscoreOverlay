...
import { collectBackfillLogFiles, mergeDungeonBestScores, scanLogFilesForBestScores } from './services/dungeon-scores';
import { UNKNOWN_HERO } from './services/hero-catalog';
...
function sendDungeonScores(): void {
  const payload: DungeonScoresPayload = { dungeonBestScores: settingsStore.getDungeonBestScores() };
  const currentWin = getLiveWindow();
  currentWin?.webContents.send('dungeon-scores', payload);
}

// Live tail: every re-parse of the current log lands here; if the parse ended on
// a counted clear (score > 0), fold it into the stored per-dungeon bests.
// Note: FinalizedDungeonState.completionSeconds carries the DUNGEON_END score
// float — the same number the in-game Dungeon Records screen shows.
// Scores are bucketed per hero: the local player is recentSkillsPlayerId (first
// party member logged at DUNGEON_START) and their className is the hero name.
function resolveLocalHeroName(payload: LogDataPayload): string {
  const data = payload?.data;
  if (!data) return UNKNOWN_HERO;
  const localPlayer = (data.players || []).find((player) => player.id === data.recentSkillsPlayerId) || null;
  const heroName = String(localPlayer?.className || '').trim();
  if (!heroName || heroName.toLowerCase().startsWith('unknown')) return UNKNOWN_HERO;
  return heroName;
}

function updateDungeonScoresFromLogData(payload: LogDataPayload): void {
  const dungeon = payload?.ok ? payload.data?.dungeon : null;
  // dungeon.data is null for non-dungeon zones: zoning to town overwrites the
  // dungeon name in parser state while the old score lingers, so without this
  // guard a bogus town entry could be recorded.
  if (!dungeon?.endedAt || !dungeon.name || !dungeon.data) return;

  const score = Number(dungeon.completionSeconds || 0);
  if (!Number.isFinite(score) || score <= 0) return;

  const merged = mergeDungeonBestScores(
    settingsStore.getDungeonBestScores(),
    [{
      timestamp: String(dungeon.endedAt),
      name: String(dungeon.name),
      tier: Number(dungeon.difficulty || 0),
      score,
      hero: resolveLocalHeroName(payload),
    }],
    settingsStore.getSeasonStartDate(),
  );
  if (!merged.changed) return;

  settingsStore.setDungeonBestScores(merged.next);
  sendDungeonScores();
}

let lastBackfilledDirectory: string | null = null;

async function runDungeonScoreBackfill(): Promise<void> {
  const directoryPath = settingsStore.getCurrentDirectoryPath();
  if (!directoryPath || directoryPath === lastBackfilledDirectory) return;
  lastBackfilledDirectory = directoryPath;

  try {
    const seasonStartDate = settingsStore.getSeasonStartDate();
    const files = collectBackfillLogFiles(directoryPath, seasonStartDate);
    if (files.length) {
      const merged = await scanLogFilesForBestScores(files, seasonStartDate, settingsStore.getDungeonBestScores());
      if (merged.changed) settingsStore.setDungeonBestScores(merged.next);
    }
  } catch {}
  sendDungeonScores();
}
...
function sendLogData(payload: LogDataPayload & { updatedAt?: string }): void {
  updateDungeonScoresFromLogData(payload);
  const currentWin = getLiveWindow();
...
function createWindow(): void {
  ...
  win.webContents.once('did-finish-load', () => {
    ...
    sendDungeonScores();
    void logDirectoryService.restoreLastLogDirectoryIfAvailable().then(() => runDungeonScoreBackfill());
    applyTrackedOverlayState(lastGameWindowState);
  });
