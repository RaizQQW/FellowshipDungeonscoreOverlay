(() => {
  ...
  function normalizeHeroDungeonMap(value: unknown): DungeonBestScores {
    ...
    Object.entries(value).forEach(([dungeonName, entry]) => {
      const name = String(dungeonName || '').trim();
      const source = entry as { score?: unknown; tier?: unknown; clearedAt?: unknown } | null;
      ...
      normalized[name] = {
        ...
        clearedAt: String(source?.clearedAt || ''),
      };
    });
    return normalized;
  }

  // Per-hero shape: { heroName: { dungeonName: entry } }. Legacy flat entries
  // (score directly on the first level) are dropped — the main process
  // rebuilds per-hero history from the logs and pushes a fresh payload.
  function normalizeDungeonBestScores(value: unknown): HeroBestScores {
    if (!value || typeof value !== 'object' || Array.isArray(value)) return {};
    const normalized: HeroBestScores = {};
    Object.entries(value).forEach(([heroName, heroEntry]) => {
      const hero = String(heroName || '').trim();
      if (!hero) return;
      const legacyScore = Number((heroEntry as { score?: unknown } | null)?.score);
      if (Number.isFinite(legacyScore)) return;
  ...
  function normalizeOverlaySettings(value: unknown): OverlaySettings {
    ...
    return {
      ...
      seasonStartDate: typeof source.seasonStartDate === 'string' && source.seasonStartDate
        ? source.seasonStartDate
        : DEFAULT_OVERLAY_SETTINGS.seasonStartDate,
  ...
  function createOverlaySettingsController(api: OverlayApi): OverlaySettingsController {
    ...
    return {
      ...
      loadDungeonScoresPanelPosition() {
        return normalizePosition(getOverlaySettings().panelPositions?.dungeonScores, DEFAULT_DUNGEON_SCORES_PANEL_POSITION);
      },
      loadDungeonBestScores() {
        return normalizeDungeonBestScores(getOverlaySettings().dungeonBestScores);
      },
      loadSkillSelections() {
      ...
      saveDungeonScoresPanelPosition(position: Point) {
        saveOverlaySettingsPatch({
          panelPositions: {
            dungeonScores: normalizePosition(position, DEFAULT_DUNGEON_SCORES_PANEL_POSITION),
          } as OverlayPanelPositions,
        });
      },
