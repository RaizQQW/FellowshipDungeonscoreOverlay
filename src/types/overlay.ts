...
export interface DungeonBestEntry {
  score: number;
  tier: number;
  clearedAt: string;
}

export type DungeonBestScores = Record<string, DungeonBestEntry>;

// Best scores are tracked per hero (character): the same account plays e.g.
// Sylvie, Helena, and Xavian, and the game counts highest key/rating per hero.
// Outer key = hero name ('Unknown' when a clear can't be attributed).
export type HeroBestScores = Record<string, DungeonBestScores>;
...
export interface DungeonScoresPayload {
  dungeonBestScores: HeroBestScores;
}
...
export interface OverlayApi {
  ...
  onDungeonScores(callback: (payload: DungeonScoresPayload) => void): void;
...
export interface RendererConstantsApi {
  ...
  DUNGEON_SCORES_PANEL_POSITION_KEY: string;
  ...
  DEFAULT_DUNGEON_SCORES_PANEL_POSITION: Point;
...
export interface DungeonScoresPanelVisibilityArgs {
...
export interface RendererPanelsApi {
  ...
  renderDungeonScoresPanel(args: RenderDungeonScoresPanelArgs): void;
  ...
  updateDungeonScoresPanelVisibility(args: DungeonScoresPanelVisibilityArgs): void;
}
...
export interface OverlaySettingsController {
  ...
  loadDungeonScoresPanelPosition(): Point;
  loadDungeonBestScores(): HeroBestScores;
  ...
  saveDungeonScoresPanelPosition(position: Point): void;
